 ...
 */
	public function handle($context)
	{
	    ...
	    if ($fd->hasfile('uploads'))
	    {
    		...
    		{
    		    foreach(new FAIterator('uploads') as $ix => $fa)
    		    { # we only support private or public in this case so there is no flag
    			$upl = R::dispense('upload');
    			$upl->savefile($context, $fa, Config::UPUBLIC, $context->user());

                $upl2 = R::dispense('uploaddetail');
                $upl2->user = $context->user();
                $upl2->upload = $upl;
                $upl2->description = $fd->post('description');
                $upl2->theme = $context->load('theme', $fd->post('theme'));
                $upl2->hidden = $fd->post('hidden');
                R::store($upl2);
    		    }
    		}
	    }
